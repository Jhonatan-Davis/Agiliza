...
if (!isset($_SESSION['usuario_id'])) {
    ...
    header("Location: " . BASE_URL . "/views/login/login.php?redirect=" . urlencode($redirect_url));
    exit();
}
...
<script>
    ...
    btnLista.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('<?= BASE_URL ?>/controllers/processar_lista_espera.php', {
                method: 'POST',
    ...
    async function carregarServicos() {
        ...
        try {
            const response = await fetch(`<?= BASE_URL ?>/controllers/AgendamentoController.php?action=dados&subaction=servicos&id_negocio=${id_negocio}`);
    ...
    async function carregarProfissionais() {
        ...
        try {
            const response = await fetch(`<?= BASE_URL ?>/controllers/AgendamentoController.php?action=dados&subaction=profissionais&id_negocio=${id_negocio}`);
   ...
   // --- 6. PASSO 3: Carregar Horários (ATUALIZADA COM LISTA DE ESPERA) ---
    async function carregarHorarios(dataSelecionada) {
        ...
        try {
            const response = await fetch(`<?= BASE_URL ?>/controllers/AgendamentoController.php?action=dados&subaction=horarios&id_negocio=${id_negocio}&id_funcionario=${funcId}&id_servico=${servicoId}&data=${dataSelecionada}`);
    ...
    btnConfirmar.addEventListener('click', async () => {
        ...
        try {
            const response = await fetch('<?= BASE_URL ?>/controllers/AgendamentoController.php?action=processar', {
                method: 'POST',
                body: formData
            });
